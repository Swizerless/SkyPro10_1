...
def test_convert_transaction_rub() -> None:
    with patch("src.external_api.requests.get") as r_mock:
        ...
        assert (
            convert_transaction_rub({"operationAmount": {"amount": "34114.93", "currency": {"code": "RUB"}}})
            == 34114.93
        )

        assert (
            convert_transaction_rub({"operationAmount": {"amount": "34114.93", "currency": {"code": "GBR"}}})
            == "Неизвестная валюта: GBR"
        )
